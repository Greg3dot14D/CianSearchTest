...
INSERT INTO AAA_TRSF_CLIENTS_IDS (LAST_NAME, FIRST_NAME, MID_NAME, 
                                  ...
                                  CONTACT_MDMID, DUL_MDMID
                                  )
       (
           SELECT SUBSTR(FULLNAME,1,INSTR(FULLNAME, ' ')-1), SUBSTR(FULLNAME, INSTR(FULLNAME, ' ')+1, INSTR(FULLNAME, ' ',1,2)-INSTR(FULLNAME, ' ')-1), SUBSTR(FULLNAME, INSTR(FULLNAME, ' ',1,2)+1, LENGTH(FULLNAME)-INSTR(FULLNAME, ' ',1,2)),
                  TO_DATE(BIRTHDAY, 'DD.MM.YYYY'), 
                  DOC_SERIES, DOC_NUMBER,
                  TRIM(s_sequence_pkg.get_next_sbllike_id), TRIM(s_sequence_pkg.get_next_sbllike_id), TRIM(s_sequence_pkg.get_next_sbllike_id), TRIM(s_sequence_pkg.get_next_sbllike_id),
                  TRIM(s_sequence_pkg.get_next_rowid), TRIM(s_sequence_pkg.get_next_rowid)
...
INSERT INTO SIEBEL.S_CONTACT (ROW_ID, CREATED, CREATED_BY, LAST_UPD, LAST_UPD_BY, DCKING_NUM, MODIFICATION_NUM, CONFLICT_ID, PAR_ROW_ID, ACTIVE_FLG, 
                              ...
                              ATTRIB_10, SBRF_LITERACY, SBRF_RESIDENT, X_SBRF_PAYROLL_CLIENT, X_PB_FLG, X_SBRF_CONTACT_WEIGHT)
       (
           SELECT PARTY_ID, sysdate, '0-1', sysdate, '0-1', 0, 1, '0', PARTY_ID, 'Y', 
                  ...
                  '������', '��������', 'PLSQL', UPPER(MID_NAME), '������', UPPER(LAST_NAME||SUBSTR(FIRST_NAME, 1, 1)||SUBSTR(MID_NAME, 1, 1)), 'Opt-Out: All Parties', '0-5220', '�������', 'N', 'Y',
                  'Y', 'Y', 'Y', 'N', 'N', 0
           FROM AAA_TRSF_CLIENTS_IDS
       );   
...
INSERT INTO SIEBEL.S_PS_CREDENTIAL (ROW_ID, CREATED, CREATED_BY, LAST_UPD, LAST_UPD_BY, MODIFICATION_NUM, CONFLICT_ID, CATEGORY_CD, CONTACT_ID, CREDENTIAL_NUM, 
       ...
       (
           SELECT PS_CREDEN_ID, sysdate, '0-1', sysdate, '0-1', 0, '0', 'Passport', PARTY_ID, DUL_MDMID, 
                  '���������� ���������', 'Active', sysdate, BIRTHDAY + 365 * 20 + 5, 'PLSQL', 'Y', UPPER(FIRST_NAME), UPPER(LAST_NAME), 
                  UPPER(MID_NAME), '21', 'AA', SUBSTR(DUL_MDMID, 3, 3)||'-'||SUBSTR(DUL_MDMID, 6, 3), '��� ������', REPLACE(DOC_SERIES, ' ', '')||REPLACE(DOC_NUMBER, ' ', ''), DOC_SERIES||' '||DOC_NUMBER
           FROM AAA_TRSF_CLIENTS_IDS
       );
       
       
INSERT INTO SIEBEL.CX_CONT_INFO (ROW_ID, PERSON_UID, LAST_NAME, FST_NAME, MID_NAME, PASSPORT_SERIES, PASSPORT_NUMBER)
       (
           SELECT PARTY_ID, CONTACT_MDMID, LAST_NAME, FIRST_NAME, MID_NAME, REPLACE(DOC_SERIES, ' ', ''), REPLACE(DOC_NUMBER, ' ', '')
           FROM AAA_TRSF_CLIENTS_IDS                                      
       );

COMMIT;

CREATE TABLE AAA_43328_CLIENTS_MDM AS
       (
           SELECT PARTY_ID, CONTACT_MDMID, LAST_NAME, FIRST_NAME, MID_NAME
           FROM AAA_TRSF_CLIENTS_IDS
       );
